...
from retrain_model.new_model import get_default_arch, network_layer_to_space, newModel
...
class Retrain_Autodeeplab(nn.Module):
    def __init__(self, args):
        ...
        if args.net_arch is not None and args.cell_arch is not None:
            network_arch, network_path, cell_arch = np.load(args.net_path), np.load(args.net_arch), np.load(args.cell_arch)
            print("Using search architecture")
        else:
            network_arch, cell_arch, network_path = get_default_arch()
            print("default architecture")

        self.encoder = newModel(network_arch, cell_arch, args.num_classes, args.layer, args.filter_multiplier, BatchNorm=BatchNorm2d, args=args)
